...
<div class="panel">
  ...
  <div class="view {tab === 'settings' ? '' : 'hidden'}">
    <div
      class="card"
      style="background: linear-gradient(135deg, var(--rsdh-accent-subtle) 0%, transparent 100%);"
    >
      <label
      ...
      {#if USE_PROXY}
        <div class="card" style="background: rgba(255,255,255,0.04);">
          ...
          >
            ...
            <span
              ><strong>Bypass Proxy (dev)</strong><br /><span class="sub"
                >Use your own OpenRouter API key instead of the hosted proxy.</span
              ></span
            >
          </label>
          {#if bypassProxy}
            <div style="margin-top: 10px;">
              <label class="sub" for="or-key-debug">API Key</label>
              <input
                id="or-key-debug"
                style="width:100%;"
                type="password"
